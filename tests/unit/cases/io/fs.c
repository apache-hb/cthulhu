...
int main()
{
...

    {
        ...
        GROUP_EXPECT_PANIC(group, "no null name", fs_physical(NULL));
        GROUP_EXPECT_PASS(group, "no return null", fs_physical("test") != NULL);
    }

    {
        ...
        fs_t *fs = fs_physical(format("%s" NATIVE_PATH_SEPARATOR "data" NATIVE_PATH_SEPARATOR "unit-test-data", OS_VALUE(const char *, cwd)));
