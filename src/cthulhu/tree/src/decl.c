...
{
    tree_t *self = decl_open(node, name, type, eTreeDeclGlobal, ctu_memdup(&resolve, sizeof(tree_resolve_info_t)));
...
tree_t *tree_open_function(const node_t *node, const char *name, const tree_t *signature, tree_resolve_info_t resolve)
{
    ...
    tree_t *self = decl_open(node, name, signature, eTreeDeclFunction, ctu_memdup(&resolve, sizeof(tree_resolve_info_t)));
...
{
    return decl_open(node, name, NULL, eTreeType, ctu_memdup(&resolve, sizeof(tree_resolve_info_t)));
}
...
{
    tree_t *self = decl_open(node, name, NULL, eTreeTypeStruct, ctu_memdup(&resolve, sizeof(tree_resolve_info_t)));
...
{
    tree_t *self = decl_open(node, name, NULL, eTreeTypeUnion, ctu_memdup(&resolve, sizeof(tree_resolve_info_t)));
    self->fields = vector_new(4);
    return self;
}
...
{
    tree_t *self = decl_open(node, name, NULL, eTreeTypeEnum, ctu_memdup(&resolve, sizeof(tree_resolve_info_t)));
