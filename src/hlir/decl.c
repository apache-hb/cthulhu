...
#define IS_AGGREGATE(hlir)                    \
    (hlis_is_or_will_be(hlir, HLIR_STRUCT) || \
     hlis_is_or_will_be(hlir, HLIR_UNION))

///
/// builder functions
///

static hlir_t *hlir_begin_aggregate_with_fields(
    const node_t *node, const char *name, vector_t *fields, hlir_kind_t type) {
...
static hlir_t *
hlir_begin_aggregate(const node_t *node, const char *name, hlir_kind_t type) {
...
static void hlir_finish(hlir_t *self, hlir_kind_t type) {
    CHECK_NULL(self);
    CTASSERT(
        hlir_is(self, HLIR_FORWARD), "hlir-finish called on non-forward hlir");
...
hlir_t *hlir_struct(const node_t *node, const char *name, vector_t *fields) {
    hlir_t *self =
        hlir_begin_aggregate_with_fields(node, name, fields, HLIR_STRUCT);
...
hlir_t *hlir_union(const node_t *node, const char *name, vector_t *fields) {
    hlir_t *self =
        hlir_begin_aggregate_with_fields(node, name, fields, HLIR_UNION);
...
hlir_t *hlir_alias(
    const node_t *node, const char *name, const hlir_t *type, bool newtype) {
...
hlir_t *
hlir_begin_global(const node_t *node, const char *name, const hlir_t *type) {
...
hlir_t *hlir_global(
    const node_t *node, const char *name, const hlir_t *type,
    const hlir_t *init) {
...
hlir_t *hlir_indexed_local(
    const node_t *node, const char *name, size_t index, const hlir_t *type) {
    hlir_t *self = hlir_new_decl(node, name, type, HLIR_LOCAL);
    self->index = index;
    return self;
}

static hlir_t *hlir_begin_function_with_locals(
    const node_t *node, const char *name, vector_t *locals,
...
hlir_t *hlir_begin_function(
    const node_t *node, const char *name, signature_t signature) {
    return hlir_begin_function_with_locals(
        node, name, vector_new(4), signature);
}
...
hlir_t *hlir_function(
    const node_t *node, const char *name, signature_t signature,
    vector_t *locals, hlir_t *body) {
    hlir_t *self =
        hlir_begin_function_with_locals(node, name, locals, signature);
...
void hlir_add_local(hlir_t *self, hlir_t *local) {
    CTASSERT(
        hlir_will_be(self, HLIR_FUNCTION),
        "hlir-add-local called on non-function hlir");
...
void hlir_build_module(
    hlir_t *self, vector_t *types, vector_t *globals, vector_t *functions) {
...
hlir_t *hlir_module(
    const node_t *node, const char *name, vector_t *types, vector_t *globals,
    vector_t *functions) {
