...
static const hlir_attributes_t kDefaultAttributes = { .linkage = LINK_INTERNAL,
                                                      .tags = DEFAULT_TAGS };
...
hlir_t *hlir_new_decl(
    const node_t *node, const char *name, const hlir_t *of, hlir_kind_t kind) {
...
hlir_t *hlir_new_forward(
    const node_t *node, const char *name, const hlir_t *of,
    hlir_kind_t expect) {
...
vector_t *closure_params(const hlir_t *self) {
    CTASSERT(
        self->type == HLIR_CLOSURE,
        "closure-params(self->type != HLIR_CLOSURE)");
...
bool closure_variadic(const hlir_t *self) {
    CTASSERT(
        self->type == HLIR_CLOSURE,
        "closure-variadic(self->type != HLIR_CLOSURE)");
...
const hlir_t *closure_result(const hlir_t *self) {
    CTASSERT(
        self->type == HLIR_CLOSURE,
        "closure-result(self->type != HLIR_CLOSURE)");
